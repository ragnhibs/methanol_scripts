...
def read_annual_mean():
    ...
    annual_mean_dry = weighted_felt.mean(dim='time')

    print(annual_mean_dry)
    annual_mean_dry.to_netcdf('results_netcdf/annual_mean_' + variable_id+'_'+table_id+'_'+model_id+'_'+project_id + '_' +experiment_id+'_'+member_id+'_'+str(year_period[0])+'_'+str(year_period[1])+'.nc')
    
    #Plot a map:
    mapplot=annual_mean_dry.plot(ax=ax,levels=levels,cmap=cmap) #,transform=ccrs.PlateCarree())
    ...
    totsum = (annual_mean_dry*areaxy).sum()*1e-9*factor
    print(totsum.values)
...
unit = {'drych3oh':'kg m-2 s-1',
        'emico':'kg m-2 s-1'}

#unit_scale = {'ppt':1e12,
#              'ppb':1e9}

level_list =  {'drych3oh':np.arange(0,2.2,0.2)*1e-10,
